...
pub trait DatabaseStorage {
    fn read_page(&mut self, page_index: u32) -> Result<PageBytes>;
    fn write_page(&mut self, data: &[u8], page_index: u32) -> Result<()>;
    ...
    fn write_file_info(&mut self) -> Result<()> {
    ...
    fn write_db_info(&mut self, db_name: &str, db_id: DatabaseFileId) -> Result<()> {
    ...
    fn write_schema_info(&mut self) -> Result<()> {
...
pub fn read_page_as<'a, T>(file: &mut dyn DatabaseStorage, page_index: PageId) -> Result<T>
where
...
impl DiskFile {
    ...
    fn seek_page_index(&mut self, page_index: u32) -> Result<()> {
...
impl DatabaseStorage for MemoryFile {
    ...
    fn allocated_page_count(&self) -> Result<PageId> {
        Ok((self.data.len() / PAGE_SIZE_BYTES as usize) as u32)
    }
}
...
mod memory_file_tests {
    ...
    #[test]
    fn test_write_page() {
        let mut file = MemoryFile::new(vec![]);
        let data = vec![1, 2, 0];
